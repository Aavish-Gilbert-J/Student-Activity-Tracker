...
def add_internship(company_name, start_date, end_date, user_id):
    conn = sqlite3.connect("data/student_tracker.db")
    cursor = conn.cursor()

    cursor.execute("INSERT INTO internship (company_name, start_date, end_date, user_id) VALUES (?, ?, ?, ?)", (company_name, start_date, end_date, user_id))
...
def display_internship_page():
    ...
    if submit_button:
        ...
        add_internship(company_name, start_date, end_date, user_id)
    ...
    if internships:
        ...
        for internship in internships:
            st.write(f"Company: {internship[1]}, Start Date: {internship[2]}, End Date: {internship[3]}")
            ...
            if st.button(f"Delete {internship[1]}"):
                # Delete internship
